import numpy as np
from pyrwr.pyrwr.rwr import RWR
...
def formart_DISandRNA(RNA_CIS, Disease_DIS, path):
    DIS = []
    RNA = []
    ...
    np.savetxt(path+'RWR_FORMART_DIS.csv', DIS)
    np.savetxt(path+'RWR_FORMART_CIS.csv', RNA)
    return DIS, RNA


def SimtoRWR(RNA_CIS, Disease_DIS, path):
    RNA_CIS = simabs(RNA_CIS)
    Disease_DIS = simabs(Disease_DIS)
    path_DIS = path+'RWR_FORMART_DIS.csv'
    path_CIS = path+'RWR_FORMART_CIS.csv'
    formart_DISandRNA(RNA_CIS, Disease_DIS, path)
